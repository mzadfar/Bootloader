...
uint8_t GF_Mult(uint8_t a, uint8_t b)
{
    ...
    for (uint8_t i = 0; i < 8; i++)
    {
        ...
        // Shift `b` down in order to look at the next LSB (worth twice as much in the
        // multiplication)
        b >>= 1;
    }
...
void AES_MixColumns(AES_Block_t state)
{
    ...
    for (size_t i = 0; i < 4; i++)
    {
        temp[0] = GF_Mult(0x02, state[i][0]) ^ GF_Mult(0x03, state[i][1]) ^ state[i][2]
                ^ state[i][3];
        temp[1] = state[i][0] ^ GF_Mult(0x02, state[i][1]) ^ GF_Mult(0x03, state[i][2])
                ^ state[i][3];
        temp[2] = state[i][0] ^ state[i][1] ^ GF_Mult(0x02, state[i][2])
                ^ GF_Mult(0x03, state[i][3]);
        temp[3] = GF_Mult(0x03, state[i][0]) ^ state[i][1] ^ state[i][2]
                ^ GF_Mult(0x02, state[i][3]);
...
void AES_InvMixColumns(AES_Block_t state)
{
    ...
    for (size_t i = 0; i < 4; i++)
    {
        temp[0] = GF_Mult(0x0e, state[i][0]) ^ GF_Mult(0x0b, state[i][1])
                ^ GF_Mult(0x0d, state[i][2]) ^ GF_Mult(0x09, state[i][3]);
        temp[1] = GF_Mult(0x09, state[i][0]) ^ GF_Mult(0x0e, state[i][1])
                ^ GF_Mult(0x0b, state[i][2]) ^ GF_Mult(0x0d, state[i][3]);
        temp[2] = GF_Mult(0x0d, state[i][0]) ^ GF_Mult(0x09, state[i][1])
                ^ GF_Mult(0x0e, state[i][2]) ^ GF_Mult(0x0b, state[i][3]);
        temp[3] = GF_Mult(0x0b, state[i][0]) ^ GF_Mult(0x0d, state[i][1])
                ^ GF_Mult(0x09, state[i][2]) ^ GF_Mult(0x0e, state[i][3]);
...
void AES_EncryptBlock(AES_Block_t state, const AES_Block_t* keySchedule)
{
    ...
    for (size_t i = 1; i < NUM_ROUND_KEYS_128; i++)
    {
        ...
        // No column mix in the last round. Not that this implementation should be considered
        // for production, but this would constitute a timing based side-channel risk
