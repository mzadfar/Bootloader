...
{
    Comms_Create_Single_Byte_Packet(&retransmitPacket, PACKET_RETRANSMIT_DATA0);
    Comms_Create_Single_Byte_Packet(&acknowledgePacket, PACKET_ACKNOWLEDGE_DATA0);
}
...
void Comms_Update(void)
{
    while (USART_Data_Available(USART3))
    {
        ...
        case commsStateCRC:
        {
            ...
            memcpy(&packetBuffer[packetWriteIndex], &tempPacket, sizeof(commsPacket_t));
...
void Comms_Create_Single_Byte_Packet(commsPacket_t* packet, uint8_t byte)
{
    ...
    packet->length = PACKET_LENGTH_BYTES;
    packet->data[0] = byte;
