...
static const uint8_t secretKey[AES_BLOCK_SIZE] = {
    0x00,
    0x01,
    0x02,
    0x03,
...
static void AES_CBC_MAC_Step(
    AES_Block_t stateAES, AES_Block_t previousStateAES, const AES_Block_t* keySchedule);
...
static void AES_CBC_MAC_Step(
    AES_Block_t stateAES, AES_Block_t previousStateAES, const AES_Block_t* keySchedule)
{
    /*CBC chain of operation*/
    for (uint8_t i = 0; i < AES_BLOCK_SIZE; i++)
    {
        ((uint8_t*)stateAES)[i] ^= ((uint8_t*)previousStateAES)[i];
    }

    AES_EncryptBlock(stateAES, keySchedule);
    memcpy(previousStateAES, stateAES, AES_BLOCK_SIZE);
}

static bool Validate_Firmware_Image(void)
{
    fwInfo_t* fwInfo = (fwInfo_t*)FW_INFO_ADDRESS;
    const uint8_t* signature = (const uint8_t*)SIGNATURE_ADDRESS;

    if (fwInfo->sentinel != FW_INFO_SENTINEL)
    {
        return false;
    }

    if (fwInfo->deviceID != DEVICE_ID)
    {
        return false;
    }

    /*    const uint8_t* startAddress = (const uint8_t*)FW_INFO_VALIDATE_ADDRESS;
        const uint32_t computedCRC = crc32(startAddress,
       FW_INFO_VALIDATE_LENGTH(fwInfo->length)); return computedCRC == fwInfo->crc32;
    */
    AES_Block_t roundKeys[NUM_ROUND_KEYS_128];
    AES_KeySchedule128(secretKey, roundKeys);

    AES_Block_t stateAES = {0};
    AES_Block_t previousStateAES = {0};

    uint8_t bytesPadded = 16 - (fwInfo->length % 16);
    if (bytesPadded == 0)
    {
        bytesPadded = 16;
    }

    memcpy(stateAES, fwInfo, AES_BLOCK_SIZE);
    AES_CBC_MAC_Step(stateAES, previousStateAES, roundKeys);

    uint32_t offset = 0;
    while (offset < fwInfo->length)
    {
        /*Are we at the point where to skip the info and signature section?*/
        if (offset == (FW_INFO_ADDRESS - MAIN_APP_START))
        {
        ...
        if ((fwInfo->length - offset) > AES_BLOCK_SIZE)
        {
            /*The regular case*/
            memcpy(stateAES, (void*)(MAIN_APP_START + offset), AES_BLOCK_SIZE);
            AES_CBC_MAC_Step(stateAES, previousStateAES, roundKeys);
        }
        else
        {
            /*The irregular case: the case of padding*/
            if (bytesPadded == 16)
            {
                ...
                // memset(stateAES, bytesPadded, AES_BLOCK_SIZE);
                memset(stateAES, AES_BLOCK_SIZE, AES_BLOCK_SIZE);
                AES_CBC_MAC_Step(stateAES, previousStateAES, roundKeys);
            }
            else
            {
                memcpy(
                    stateAES,
                    (void*)(MAIN_APP_START + offset),
                    (AES_BLOCK_SIZE - bytesPadded));
                memset(
                    (void*)(stateAES) + (AES_BLOCK_SIZE - bytesPadded),
                    bytesPadded,
                    bytesPadded);
                AES_CBC_MAC_Step(stateAES, previousStateAES, roundKeys);
            }
    ...
    return memcmp(signature, stateAES, AES_BLOCK_SIZE) == 0;
}
