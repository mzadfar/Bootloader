...
int main(void)
{
    ...
    fwInfo_t* fwInfo1 = (fwInfo_t*)FW_INFO_ADDRESS;
    const uint8_t* startAddress1 = (const uint8_t*)FW_INFO_VALIDATE_ADDRESS;
    const uint32_t fwInfo1Length = FW_INFO_VALIDATE_LENGTH(fwInfo1->length);
    const uint32_t computedCRC1 = crc32(startAddress1, fwInfo1Length);
    for (uint8_t i = 0; i < 4; i++)
    {
        USART_Write_Byte(USART3, (computedCRC1 >> (i << 3)));
    }
    for (uint8_t j = 0; j < 4; j++)
    {
        USART_Write_Byte(USART3, (fwInfo1Length >> (j << 3)));
    }
    delayUS(DEFAULT_TIMEOUT << 1);
...
static bool Validate_Firmware_Image(void)
{
    ...
    const uint32_t computedCRC = crc32(startAddress, FW_INFO_VALIDATE_LENGTH(fwInfo->length));
    return computedCRC == fwInfo->crc32;
}
