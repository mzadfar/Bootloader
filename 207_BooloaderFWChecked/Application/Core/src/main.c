...
int main(void) {
  Vector_Setup(); /*Tell main code here to start from main vector table, and not
  ...
  while (1) {
    ...
    // void USART_Write(USART_TypeDef * USARTx, uint8_t * data, const uint32_t
    // length); uint32_t USART_Read(USART_TypeDef * USARTx, uint8_t * data,
